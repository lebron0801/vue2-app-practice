...
const json = (data: AjaxRequestModel) => {
	...
	data.body = { ...common().baseConfig, ...{ data: temp.body, method: method, sign: 'dsad8sdsadas7d98' } };
...
const form = (data: AjaxRequestModel) => {
	data = Object.assign(data, {
		...
		headers: Object.assign(common().headers, { 'Content-Type': 'application/x-www-form-urlencoded' })
	});
...
export default async (options: AjaxRequestModel): Promise<CommonAjaxReturnDataModel> => {
	...
	if (config.ajax.type === 2) {
		...
		if (process.env.NODE_ENV == 'development' || process.env.NODE_ENV == 'mock' || process.env.NODE_ENV == 'sit') {
