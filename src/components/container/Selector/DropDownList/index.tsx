	...
	private handleChange(value: any) {
		...
		if (this.multiple) {
			callData = this.copyValue.map((item: any) => {
				return { [this.replaceFields.key]: item.key, [this.replaceFields.label]: item.label };
			});
		} else {
			if (value) {
				callData = [{ [this.replaceFields.key]: value.key, [this.replaceFields.label]: value.label }];
			} else {
				callData = [];
			}
		}
