	...
	validateRequired(val: boolean) {
		if (val) {
			this.validator.required = {
				required: true,
				message: `${this.data.type == 'imgupload' ? this.data.options.alise : this.$t(this.data.name)}不能为空`,
				trigger: ['blur', 'change']
			};
		} else {
				...
				{this.show && (
					<a-form-model label-col={{ span: 0 }} wrapper-col={{ span: 24 }}>
						...
						{(this.data.type == 'select' ||
							...
							<a-form-model-item>
								<div class="feild-item">
									<a-switch vModel={this.data.options.multiple} onChange={this.handleSelectMuliple} />
								</div>
							</a-form-model-item>
						]}
