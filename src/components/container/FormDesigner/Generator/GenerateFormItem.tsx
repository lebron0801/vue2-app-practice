				...
				{this.type ? (
					<div>
						...
						{widgetType == 'switch'
							...
							: null}
						{widgetType == 'radio'
							? (widget.options.remote ? widget.options.remoteOptions : widget.options.options).find(
									(item: any) => item.value == this.current
							  )?.label
							: null}
						...
						{widgetType == 'customSelector' ? this.current.map((item: any) => item.label).join(',') : null}
					...
					>
						...
						{widget.type == 'radio' && (
							...
							>
								{(widget.options.remote ? widget.options.remoteOptions : widget.options.options).map(
						...
						{widget.type == 'html' && <div domPropsInnerHTML={this.current}></div>}
						...
						on={{
							...
							close: (data: any) => {
								if (widget.options.multiple) {
									this.options = data.map((item: any) => ({ key: item.id, label: item.name }));
