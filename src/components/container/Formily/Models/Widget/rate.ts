export type RateModel = {
	...
	componentProperties: {
		allowHalf: boolean;
		allowClear: boolean;
		tooltips: { tips: string; tipsLangKey: string }[];
		count: number;
		onChange: string;
		onFocus: string;
		onBlur: string;
...
const RateModel: RateModel = {
	...
	componentProperties: {
		...
		/**
		 * @name 提示信息
		 * @description 鼠标移到star上显示的提示内容，理论上有几个star就有几条项
		 * @param {Array.<object>} tooltips
		 * @param {string} tooltips[].tips 默认提示文字
		 * @param {string} tooltips[].tipsLangKey 提示文字国际化标识
		 */
		tooltips: [],
		/**
		 * @name 总数
		 * @description 显示几个star
		 * @type {number}
		 * @default 5
		 */
		count: 5,
