		...
		return (
			<div
				class={classnames('widget-form-wrapper', 'custom-horizontal-wrapper', {
					active: this.select && this.select.config?.key === this.data.config.key
				})}
			>
					...
					props={
						this.data.config.layout === 'horizontal'
							? {
									...
									wrapperCol: { span: this.data.config.wrapperCol }
							  }
							: null
					}
					layout={this.data.config.layout}
					style={{ width: '100%', height: '100%' }}
					class={classnames(this.data.config.customClass)}
				>
					<Draggable
